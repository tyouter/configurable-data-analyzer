...
"""
Rednote KPI 指标分析 - 修复版 v2.0 (适配v2数据集)
...
def load_and_prepare_data():
    print("=" * 70)
    print("Rednote KPI 指标分析 (修复版 v2.0 适配v2数据集)")
    ...
    df['start_time_dt'] = pd.to_datetime(df['start_time_nano'], errors='coerce')
    df['end_time_dt'] = pd.to_datetime(df['end_time_nano'], errors='coerce')
    df['date'] = df['start_time_dt'].dt.date
    df['hour'] = df['start_time_dt'].dt.hour
    df['weekday'] = df['start_time_dt'].dt.dayofweek

    if 'start_time_dt' in df.columns and 'end_time_dt' in df.columns:
        df['duration_sec'] = (df['end_time_dt'] - df['start_time_dt']).dt.total_seconds()
        df.loc[df['duration_sec'] < 0, 'duration_sec'] = np.nan

    valid_dates = df[df['start_time_dt'].notna()]
    if not valid_dates.empty:
        date_min = valid_dates['start_time_dt'].min()
        date_max = valid_dates['start_time_dt'].max()
...
def calculate_app_metrics(df, total_users):
    ...
    valid_df = df[df['start_time_dt'].notna()]

    app_open_events = df[df['span_name'].str.contains('app_show|app_launch|pageshow', case=False, na=False)]
    ...
    ai_first_time_users = df[df['span_name'].str.contains('ai_guide.*click|ai.*generate', case=False, na=False)]
    ai_first_user_count = ai_first_time_users['reduser_id'].nunique()

    discovery_exposure = df[df['span_name'].str.contains('discovery.*show|discovery.*exposure', case=False, na=False)]
    ...
    app_metrics = {
        ...
        'exposure_duration_sec': {
            'value': None,
            'note': '[C-001 FIXED] 数据源限制: start_time_nano == end_time_nano，无法计算真实停留时长',
    ...
    print(f"  ⚠️ 曝光时长: 数据源不包含此信息 (start==end)")
...
def calculate_porsche_metrics(df, total_users):
    ...
    porsche_events = df[df['span_name'].str.contains('porsche|ps_', case=False, na=False)]
...
def calculate_discovery_metrics(df, total_users):
    ...
    discovery_events = df[df['span_name'].str.contains('discovery', case=False, na=False)]
...
def calculate_ai_guide_metrics(df, total_users):
    ...
    ai_click_events = df[df['span_name'].str.contains('ai_guide_button_click|generated_travel_guide', case=False, na=False)]
    ai_click_users = ai_click_events['reduser_id'].nunique() if len(ai_click_events) > 0 else 0
    total_ai_clicks = len(ai_click_events)

    ai_generated_events = df[df['span_name'].str.contains('guide_generated|travel_guide_generated', case=False, na=False)]
    ...
    ai_view_events = df[df['span_name'].str.contains('trival_guide_page_detail|guide_result_view', case=False, na=False)]
    ai_viewed_count = len(ai_view_events)
    ai_viewed_users = ai_view_events['reduser_id'].nunique() if len(ai_view_events) > 0 else 0

    ai_share_events = df[df['span_name'].str.contains('guide_share|share_guide', case=False, na=False)]
    ai_shared_count = len(ai_share_events)
    ai_share_users = ai_share_events['reduser_id'].nunique() if len(ai_share_events) > 0 else 0

    app_open_users = df[df['start_time_dt'].notna()]['reduser_id'].nunique()
    ...
    ai_metrics = {
        ...
        'ai_funnel': {
            'button_shown': {'events': len(df[df['span_name'].str.contains('ai_guide.*show|ai_guide.*display', case=False, na=False)]), 'users': df[df['span_name'].str.contains('ai_guide.*show|ai_guide.*display', case=False, na=False)]['reduser_id'].nunique()},
...
def calculate_share_metrics(df, total_users):
    ...
    share_gen_events = df[df['span_name'].str.contains('share_gen|generate_share', case=False, na=False)]
    share_use_events = df[df['span_name'].str.contains('share_use|use_share|share_click', case=False, na=False)]
...
def calculate_detailed_insights(df, total_users):
    ...
    porsche_analysis = {}
    porsche_evts = df[df['span_name'].str.contains('porsche', case=False, na=False)]
    porsche_analysis['total_events'] = len(porsche_evts)
    poi_card_shown = df[df['span_name'].str.contains('poi_card.*show|cardshow.*poi', case=False, na=False)]
    porsche_analysis['poi_card_shown'] = len(poi_card_shown)
    poi_card_clicked = df[df['span_name'].str.contains('poi_card.*click|poi.*click', case=False, na=False)]
    ...
    search_analysis = {}
    search_evts = df[df['span_name'].str.contains('search', case=False, na=False)]
    ...
    content_analysis = {}
    post_views = df[df['span_name'].str.contains('post.*view|post.*show|pageshow.*post', case=False, na=False)]
    content_analysis['total_content_views'] = len(post_views)
    
    like_events = df[df['span_name'].str.contains('like', case=False, na=False)]
    content_analysis['likes'] = len(like_events)
    
    save_events = df[df['span_name'].str.contains('save|collect|bookmark', case=False, na=False)]
    ...
    normal_posts = df[df['span_name'].str.contains('normal.*post|post.*normal', case=False, na=False)]
    video_posts = df[df['span_name'].str.contains('video', case=False, na=False)]
    content_analysis['post_types'] = {
        'normal': len(normal_posts),
        'video': len(video_posts)
    }
    
    video_play = df[df['span_name'].str.contains('video.*play|play.*video', case=False, na=False)]
    content_analysis['video_events'] = len(video_play)
    insights['content_analysis'] = content_analysis

    map_analysis = {}
    map_evts = df[df['span_name'].str.contains('map', case=False, na=False)]
    map_analysis['total_map_events'] = len(map_evts)
    fullscreen = df[df['span_name'].str.contains('fullscreen.*map|map.*fullscreen', case=False, na=False)]
    ...
    user_stats = valid_df.groupby('reduser_id').agg(
        ...
        unique_pages=('span_name', 'nunique'),
...
def main():
    ...
    results['core_overview'] = {
        'app_open_total': int(len(df[df['start_time_dt'].notna()].groupby(['reduser_id', 'date']))),
        'active_users': int(df[df['start_time_dt'].notna()]['reduser_id'].nunique()),
        'poi_interactions': int(len(df[df['span_name'].str.contains('poi', case=False, na=False)])),
        'unique_poi_count': int(df[df['span_name'].str.contains('poi', case=False, na=False)]['span_name'].nunique()),
    ...
    json_output = os.path.join(OUTPUT_DIR, 'kpi_metrics_v3.json')
